				<div class="uk-width-1-4@m">
                    <div class="uk-panel uk-card uk-text-center">
                        <img class="uk-border-circle" width="120" height="120" src="data:image/svg+xml;base64," alt="">
                        <h3>Hoşgeldin <?php
							// Oturum degiskeni kontrol et
							if(isset($_SESSION['kullanici'])){
								
								// Oturum acildi yazisini ve cikis linki goster
								print $_SESSION['ad'] . " "; 
								print $_SESSION['soyad'];

							} else {
								print "Mısafır";
							}

						?></h3>
                    </div>
                    <hr>
                    <div class="uk-section uk-section-primary uk-light">
                        <div class="uk-text-center">
                            <h3 class="uk-panel-title">Kategoriler</h3>
                            <ul class="uk-list uk-list-divider">
                            	<?php 

                            	$kategoriler_liste = kategori_goster();

                            	if($kategoriler_liste) {
                            		foreach ($kategoriler_liste as $kategori) { ?>
                            		<li><a href="index.php?goster=kategori&amp;id=<?php print $kategori['id']; ?>"><?php print $kategori['adi']; ?></a></li>
    					       <?php } } ?>
                            </ul>
                        </div>
                    </div>
                    <div class="uk-section uk-section-muted">
                        <div class="uk-text-center">
                            <h3 class="uk-panel-title">Yazarlar</h3>
                            <ul class="uk-list uk-list-divider">
                            	<?php 

                            	$yazar_liste = yazar_goster();

                            	if($yazar_liste) {
                            		foreach ($yazar_liste as $yazar) { ?>
                            		<li><a href="index.php?goster=yazar&amp;id=<?php print $yazar['id']; ?>"><?php print $yazar['adi']; ?></a></li>
    					       <?php } } ?>
                            </ul>
                        </div>
                    </div>
                </div>